...
from model import BAM_h
from model import BAM_h2
from model import BAM_h20
from model import BAM_h22
from model import BAM_h8
from model import BAM_h10
...
seed_array = [510,510,510,510,510]
# seed_array = [382,382,382,382,382]
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            output, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls,num = iter_num)
