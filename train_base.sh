...
exp_dir=/output/${dataset}/${arch}/${exp_name}/${net}
...
CUDA_VISIBLE_DEVICES=${GPU_ID} python3 -m torch.distributed.launch --nproc_per_node=2 --master_port=1234 train_base.py \
        --config=${config} \
        --arch=${arch} \
