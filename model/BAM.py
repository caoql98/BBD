...
class OneModel(nn.Module):
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        with torch.no_grad():
            ...
            # res = F.interpolate(query_feat_0 , size=(query_feat_1.size(2),query_feat_1.size(3)), mode='bilinear', align_corners=True)
            # query_feat_1 = query_feat_1+res 

            query_feat_2 = self.layer2(query_feat_1)

            # res = F.interpolate(query_feat_0 , size=(query_feat_2.size(2),query_feat_2.size(3)), mode='bilinear', align_corners=True)
            # query_feat_2 = query_feat_2+res 

            query_feat_3 = self.layer3(query_feat_2)

            # res = F.interpolate(query_feat_0 , size=(query_feat_3.size(2),query_feat_3.size(3)), mode='bilinear', align_corners=True)
            # query_feat_3 = query_feat_3+res 

            query_feat_4 = self.layer4(query_feat_3)

            # res = F.interpolate(query_feat_0 , size=(query_feat_4.size(2),query_feat_4.size(3)), mode='bilinear', align_corners=True)
            # query_feat_4 = query_feat_4+res 
